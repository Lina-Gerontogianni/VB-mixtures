##############----------------------------------------------------------------------------------------------
#### main AVB function for the DP Poisson Mixture Model + extra helpful functions
##############----------------------------------------------------------------------------------------------
...
vb_dppoismix <- function(Y, M, phi_0, a_0, b_0, delta, phi, a, b, Temp, max_iterations, epsilon=1e-4, printL=FALSE)
{
  ...
  for (i in 2:max_iterations) 
  {
    ...
    log_rho <- N.E.logw + N.E.log1_wj  + Y %*% E.loglambda - N.E.lambda  
    ...
    l3 <- sum(log( phi_0) + (phi_0 ) * E.log1_w + lbeta(delta,phi) - (delta -1) * E.logw - (phi)*E.log1_w)
  ...
  object <- structure(list(phi=phi, delta=delta, a=a, b=b, r=r, pi_M=pi_M, E.lambda=E.lambda, L=L[2:i]))
  return(object)
} 
##############----------------------------------------------------------------------------------------------
...
Temp <- rep(1,iter)
# the model
model_dppois <- vb_dppoismix(Y, M, phi_0, a_0, b_0, delta,phi, a, b, Temp=Temp, max_iterations=iter, epsilon=1e-10, printL=T)

## ELBO plot
# truncate the initial elbo iterations 
o<-1
Elbo <- data.frame(elbo=model_dppois$L[o:length(model_dppois$L)], iter=o:length(model_dppois$L))
library(ggplot2)

pel <- ggplot(data=Elbo,aes(x=iter,y=elbo)) +xlab("iterations")+ylab("ELBO") + labs(title="ELBO of AVB-DP-PoisMix") 
...
var_weights <- model_dppois$pi_M[model_dppois$pi_M>0.003]
cat("Variational weights:", var_weights, "\n")

var_par<- as.matrix(model_dppois$E.lambda[ ,model_dppois$pi_M>0.003])
